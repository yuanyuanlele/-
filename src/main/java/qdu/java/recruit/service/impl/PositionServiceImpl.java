...
//        PageInfo<PositionCompanyBO> pageInfo = new PageInfo<>(recList);       0-6 6-12 12-18
    ...
    @Override
    public PageInfo<PositionEntity> findByPage(int page, int limit) {
        int total = positionMapper.countAll();
        PageHelper.startPage(page, limit);
        List<PositionEntity> posList = positionMapper.listPosAll();
        PageInfo<PositionEntity> pagination = new PageInfo<>(posList);
        pagination.setTotal(total);
        return pagination;
    }
    ...
    @Override
    public ArrayList<PositionEntity> listPosPage() {
        return positionMapper.listPosPage();
    }
